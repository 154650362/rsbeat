...
//增加一个处理函数，兼容127.0.0.1：6379和127.0.0.1：6379：myredis两种配置
func ipconvert(ipPortName string) string  {
	logp.Info("ipPortName %v convert to ipPort", ipPortName)
	s := strings.Split(ipPortName,":")
	if len(s) == 3 {
		ipPort := s[0] + ":" + s[1]
		return ipPort
	}
...
func New(b *beat.Beat, cfg *common.Config) (beat.Beater, error) {
	...
	for _, ipPort := range config.Redis {
		poolList[ipPort] = poolInit(ipconvert(ipPort), config.SlowerThan, config.SlowerLen)
...
func (bt *Rsbeat) redisc(beatname string, init bool, c redis.Conn, ipPort string) {
	...
	r, _ := redis.Values(c.Receive())  // reply from ROLE
	role := string(r[0].([]byte))
	...
	for _, i := range reply {
		...
		event := common.MapStr{
			...
			"ip_port":        ipPort,
			"role":           role,   //增加记录角色，master or slave
		}
...
func poolInit(server string, slowerThan int, slowerLen int ) *redis.Pool {
	return &redis.Pool{
		MaxIdle:     3,
		MaxActive:   3,
		...
		Dial: func() (redis.Conn, error) {
			...
			c.Send("CONFIG", "SET", "slowlog-max-len", slowerLen)
			c.Send("SLOWLOG", "RESET")
			r, err := c.Do("EXEC")
